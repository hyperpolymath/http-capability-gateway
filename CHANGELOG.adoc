...
All notable changes to `+http-capability-gateway+` will be documented in
this file.
...
* feat(perf): Phase D benchmark harness scaffold (standards#99 / #91
channel) (#12)
* feat(policy): catalog mode — auto-policy from BoJ cartridge.json
* feat(p2): bug fixes, unit tests, P2 productization docs
* feat(p1): gateway hardening — bug fixes, concurrency tests,
benchmarks, docs
...
* perf(d-2): wire in-process loopback backend so proxy-200 scenario
measures real dial-and-read (#14)
