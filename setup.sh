...
# ── Main ──
main() {
    printf "%s=== http-capability-gateway Setup ===%s\n\n" "$BOLD" "$RESET"
    ...
    if [ ! -f "Justfile" ] && [ ! -f "justfile" ]; then
        ...
        info "Clone first: git clone https://github.com/hyperpolymath/http-capability-gateway.git"
        info "Then: cd http-capability-gateway && ./setup.sh"
        exit 1
    fi
